...
var displayMap = exports.displayMap = function displayMap(locations) {
  ...
  var map = L.map('map', {
    ...
    dragging: true,
    touchZoom: false
  }).setView([locations[0].coordinates[1], locations[0].coordinates[0]], // [lat, lng]
  10); // Add the OpenStreetMap tile layer
  ...
  locations.forEach(function (loc) {
    ...
    var marker = L.marker([loc.coordinates[1], loc.coordinates[0]], // [lat, lng]
    {
      icon: customIcon
    }).addTo(map); // Bind a popup with location info

    marker.bindPopup("<p class=\"marker-popup\">Day ".concat(loc.day, ": ").concat(loc.description, "</p>")); // Extend the bounds for fitting the view
    ...
    return _regeneratorRuntime().wrap(function _callee$(_context) {
      while (1) switch (_context.prev = _context.next) {
        ...
        case 7:
          _context.prev = 7;
          _context.t0 = _context["catch"](0);
          (0, _alerts.showAlert)('error', _context.t0.response && _context.t0.response.data && _context.t0.response.data.message || 'Signup failed!');
...
function () {
  ...
  _regeneratorRuntime().mark(function _callee(tourId) {
    ...
    return _regeneratorRuntime().wrap(function _callee$(_context) {
      while (1) switch (_context.prev = _context.next) {
        case 0:
          stripe = Stripe('pk_test_51R6UQfBbyTOq1z3MRk4EHo7Yv863L7RDOfy2zcPQo28BGz85AF0uATHwWkXXvJSV37CIJxLZJagweD3ZQHsbHHaO00jS2hbs2U'); // 1) Get checkout session from API
...
if (bookBtn) {
  bookBtn.addEventListener('click', function (e) {
    e.target.textContent = 'Processing...'; // This correctly finds the tourId from the button's data attribute

    var tourId = e.target.closest('#book-tour').dataset.tourId; // This calls the function defined above with the correct ID
...
var signupForm = document.querySelector('.form--signup'); // ✅ Select signup form
